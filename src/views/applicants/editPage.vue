<template>
  <div id="editApplicant">
    <br />
    <h1 style="text-align: center; color: rebeccapurple">Редактировать Соискателя</h1>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      axios
        .put(`http://176.119.158.122:3000/applicants/${this.$route.params.id}`, formData, {
          headers: {
    ...
    async fetchApplicantInfo() {
      try {
        const response = await axios.get(
          `http://176.119.158.122:3000/applicants/${this.$route.params.id}`
        )
        const applicant = response.data[0]
        this.form.surname = applicant.Фамилия
        this.form.name = applicant.Имя
        this.form.patronymic = applicant.Отчество
        this.form.education = applicant.Образование
        this.form.specialty = applicant.Специальность
        this.form.birth_date = applicant.Дата_Рождения
        this.form.phone = applicant.Телефон
        this.form.email = applicant.Email
        this.form.photo = applicant.Фото
